...
namespace Neosoft.FAMS.Persistence.Repositories
{
    public class AdvertisementRepo : BaseRepository<AdvertisementDetail>, IAdvertisementRepo
    {
        ...
        {
            var data =  _dbContext.AdvertisementDetails.Where(p => p.IsDeleted == false && p.EndDate >= DateTime.Now).ToList();
            return data;
        }
        ...
        public async Task<List<AdvertisementDetail>> GetAllAsync()
        {
            ...
            var temp = _dbContext.AdvertisementDetails.Where(p => p.IsDeleted == false && p.EndDate >= DateTime.Now);
